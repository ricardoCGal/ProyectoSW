<?php
    ...
    // Verifica si se proporcionó un ID de publicación válido
    if (isset($_GET['id']) && is_numeric($_GET['id'])) {
        $publicacion_id = $_GET['id'];

        // Recuperamis la información de la publicación desde la base de datos
        $query = 'SELECT * FROM publicaciones WHERE id = :id';
        $statement = $conn->prepare($query);
        $statement->bindParam(':id', $publicacion_id);
        ...
        // Redirige a la página principal si no se proporciona un ID válido
    ...
    <div class="class-elem2" >
        ...
        <?php if ($publicacion): ?>
            <div id="container" class="visualizacion<?= $publicacion['tipo_visualizacion']; ?>">
                ...
                <figure id="imgs">
                    <img class="imag2" src="<?= $publicacion['imagen']; ?>" alt="<?= $publicacion['titulo']; ?>">
                </figure>
